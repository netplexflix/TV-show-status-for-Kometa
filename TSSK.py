...
# Import from modules
from tssk.constants import VERSION, IS_DOCKER, GREEN, ORANGE, BLUE, RED, RESET
from tssk.config_loader import (
    load_config, 
    load_localization, 
    ensure_output_directory,
    get_config_section
)
from tssk.utils import check_for_updates
from tssk.sonarr import (
    process_sonarr_url,
    get_sonarr_series_and_tags
)
from tssk.finders import (
...
from tssk.yaml_generators import (
...
def main():
    ...
    localization = load_localization('config/localization.yml')
    
    # Check if localization file was loaded
    localization_file_path = 'config/localization.yml'
    try:
        with open(localization_file_path, 'r', encoding='utf-8') as f:
            print(f"{GREEN}Localization file found and loaded{RESET}\n")
    except FileNotFoundError:
        print(f"{ORANGE}No localization file found, using English defaults{RESET}\n")
    except Exception:
        print(f"{ORANGE}Localization file found but could not be loaded, using English defaults{RESET}\n")
